...
export class MapGenerator {
	...
	generateTerrain() {
		this.terrain = Array.from({ length: this.height }, () => Array(this.width).fill(0));
		...
		console.log('width:', this.terrain[0].length);
		console.log('height:', this.terrain.length);
	}
	...
	generateRectRoom(): Room {
		const roomWidth =
			Math.floor(Math.random() * (this.RECT_ROOM_MAX_SIZE - this.RECT_ROOM_MIN_SIZE + 1)) +
			this.RECT_ROOM_MIN_SIZE;
		const roomHeight =
			Math.floor(Math.random() * (this.RECT_ROOM_MAX_SIZE - this.RECT_ROOM_MIN_SIZE + 1)) +
			this.RECT_ROOM_MIN_SIZE;

		const terrain: number[][] = Array.from({ length: roomHeight }, () => Array(roomWidth).fill(1));
		...
		this.printAsString(room.terrain);
	...
	generateCircularRoom(): Room {
		const radius =
			Math.floor(
				Math.random() * (this.CIRCULAR_ROOM_MAX_RADIUS - this.CIRCULAR_ROOM_MIN_RADIUS + 1)
			) + this.CIRCULAR_ROOM_MIN_RADIUS;
		const diameter = 2 * radius;
		const terrain: number[][] = Array.from({ length: diameter }, () => Array(diameter).fill(0));
		...
		for (let y = 0; y < diameter; y++) {
			for (let x = 0; x < diameter; x++) {
				...
				if (Math.sqrt(Math.pow(x - radius, 2) + Math.pow(y - radius, 2)) < radius - 1) {
					terrain[y][x] = 2;
				}
			}
		}
	...
	mergeRoomTerrain(room: Room) {
		for (let y = 0; y < room.height; y++) {
			for (let x = 0; x < room.width; x++) {
				if (room.terrain[y][x] !== 0) this.terrain[room.y + y][room.x + x] = room.terrain[y][x];
			}
		}
	}
