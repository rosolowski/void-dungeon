...
import { SkillRarity, type Skill, type TempCombatStats } from '$lib/class/Skill';
...
export const skillsArray: Skill[] = [
	{
		...
		effect: (source: Character, target?: Character | Entity) => {
			...
			const damage = Math.floor(source.stats.damage * 0.2 + source.stats.fireDamage);
	...
	},
	{
		...
		effect: (source: Character, target?: Character | Entity) => {
			...
			source.stats.hp = Math.min(source.stats.hp + healAmount, source.stats.maxHp);
			return {
				heal: healAmount,
				damageDone: damage,
	...
	},
	{
		...
		description: 'Convert all status effect stacks into healing. Heal for 5 HP per stack.',
		manaCost: 40,
		cooldown: 12,
		minLevel: 20,
	...
	},
	{
		id: 'blood_ritual',
		name: 'Blood Ritual',
		description: 'Sacrifice 20% of max HP to restore 10% of max mana. Can kill you.',
		manaCost: 0,
		cooldown: 8,
		minLevel: 15,
		rarity: SkillRarity.RARE,
		...
		effect: (source: Character) => {
			...
			source.stats.mana = Math.min(source.stats.mana + manaGain, source.stats.maxMana);
			return {
				heal: -hpCost,
				damageDone: 0,
				effectsApplied: { fire: 0, poison: 0, cold: 0, light: 0, void: 0 },
	...
	},
	{
		id: 'void_embrace',
		name: 'Void Embrace',
		description: 'Passively increase your void damage by 1% for every 10 points of missing HP.',
		manaCost: 0,
		cooldown: 0,
		minLevel: 25,
		targetType: 'passive',
	...
	},
	{
		...
		description:
			'Deal damage equal to 200% of your missing HP. Costs 10% of your current HP to use.',
		manaCost: 20,
		cooldown: 6,
		minLevel: 25,
		rarity: SkillRarity.EPIC,
		...
		effect: (source: Character, target?: Character | Entity) => {
			...
			const hpCost = Math.floor(source.stats.hp * 0.1);
			source.stats.hp -= hpCost;
			target.stats.hp -= damage;
			return {
				heal: -hpCost,
	...
	},
	{
		...
		effect: (source: Character, target?: Character | Entity) => {
			...
			const highestStackType = Object.entries(stacks).reduce((a, b) => (a[1] > b[1] ? a : b))[0];
		...
		effect: (source: Character, target?: Character | Entity) => {
			...
			for (let i = 0; i < 3; i++) {
				...
				if (Math.random() < (i + 1) * 0.33) {
					...
					target.stats[`${effect}Status` as keyof typeof target.stats] += 3 as number;
				}
			}
			target.stats.hp -= totalDamage;
			return {
	...
	},
	{
		...
		modifyStats: (source: TempCombatStats) => {
			...
			source.armor = Math.max(0, Math.floor(source.armor * (1 - damageIncrease)));
		}
	},
	{
		id: 'frenzied_tempo',
	...
	},
	{
		...
		effect: (source: Character) => {
			...
			source.stats.mana = Math.min(source.stats.mana + 20, source.stats.maxMana);
